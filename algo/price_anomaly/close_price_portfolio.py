...
def high_low_price_portfolio():
    ...
    for i in range(return_matrix.shape[0] // 21 + 1):
        tmp_return = return_matrix[i * 21: (i + 1) * 21]
        tmp_price = price_matrix.iloc[i * 21]
        ...
        medium_returns = pd.concat([medium_returns, pd.DataFrame(tmp_return[medium_x_stocks].mean(axis=1), columns=["medium"])],
                                axis=0)
    ...
    medium_x_stocks_pv = (1 + medium_returns).cumprod()
